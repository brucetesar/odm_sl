...
Find.find(SOURCE) do |path|
  ...
  Find.prune if File.basename(path) == '.gitattributes'
  ...
  Find.prune if File.basename(path) == '.rubocop.yml'
  Find.prune if File.basename(path) == 'cucumber.yml'
  ...
  Find.prune if path =~ %r{bin/main\.rb$}
  ...
  if File.directory?(path)
    ...
    content_string = File.read(path, mode: 'rb')
    ...
    File.write(target_path, trans_string, mode: 'wb')
  end
end
