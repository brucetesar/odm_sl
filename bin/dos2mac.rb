...
# file have been converted to MacOS-style format.
# DOS: CR+LF; nothing at end of file.
# MacOS: CR; CR needed at end of file.
...
# and then converts it to the MacOS newline convention (CR only).
def string2mac(str)
  univ_str = str.encode(str.encoding, universal_newline: true)
  univ_str.encode(str.encoding, cr_newline: true)
end

# Define source and target project directories
SOURCE = ODL::PROJECT_DIR
target_relative_path = File.join(SOURCE, '../odm_sl_mac')
...
Find.find(SOURCE) do |path|
  ...
  if File.directory?(path)
    ...
    # Transcode the string into MacOS line terminator format.
    trans_string = string2mac(content_string)
