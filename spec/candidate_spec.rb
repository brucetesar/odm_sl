...
RSpec.describe Candidate do
  ...
  context 'with no constraints' do
    before do
      @constraint_list = []
      @candidate = described_class.new(input, output, @constraint_list)
    end
    ...
    it 'returns the output' do
      expect(@candidate.output).to eq output
    end
  ...
  context 'with two constraints' do
    ...
    context 'with a candidate with identical violations' do
      let(:input2) { instance_double(Input, 'input2') }
      let(:output2) { instance_double(Output, 'output2') }

      before do
        @candidate2 = described_class.new(input2, output2, @constraint_list)
    ...
    context 'with a candidate with different violations' do
      let(:input2) { instance_double(Input, 'input2') }
      let(:output2) { instance_double(Output, 'output2') }
    ...
    context 'with a duplicate is made' do
