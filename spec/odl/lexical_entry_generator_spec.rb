...
RSpec.describe ODL::LexicalEntryGenerator do
  ...
  context 'when generating roots of length 1 numbered from 1' do
    ...
    before do
      ...
      allow(uf_generator).to receive(:underlying_forms).with(1)\
                                                       .and_yield(uf1)\
                                                       .and_yield(uf2)
      @generator = described_class.new(uf_generator)
  ...
  context 'when generating suffixes of length 2 numbered from 5' do
    ...
    before do
      ...
      allow(uf_generator).to \
        receive(:underlying_forms).with(2)\
