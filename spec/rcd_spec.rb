...
# The specs for class Rcd make use of a spec helper, QuickErc.
...
RSpec.describe Rcd do
  ...
  context 'with an empty ERC list and 2 constraints' do
    ...
    before do
      ...
      @rcd = described_class.new(erc_list)
    end
    ...
    it 'returns an ErcList of ERCs passed in' do
      expect(@rcd.erc_list).to be_a(ErcList)
    end
  ...
  context 'when calling Rcd.rankable?' do
    ...
    context 'when the constraint prefers a loser' do
      ...
      it 'returns false' do
        expect(described_class.rankable?(con, @ercs)).to be false
      end
    end
    ...
    context 'when the constraint prefers no losers' do
      ...
      it 'returns true' do
        expect(described_class.rankable?(con, @ercs)).to be true
      end
    end
  end

  context 'when calling Rcd.explained?' do
    let(:erc) { double('erc') }
    ...
    context 'when the erc is preferred by one of the constraints' do
      ...
      it 'returns true' do
        expect(described_class.explained?(erc, @constraints)).to be true
      end
    end
    ...
    context 'when the erc is not preferred by any of the constraints' do
      ...
      it 'returns false' do
        expect(described_class.explained?(erc, @constraints)).to be false
      end
    end
  end

  context 'when calling Rcd.rank_next_stratum' do
    context 'with one newly ranked constraint' do
      ...
      before do
        ...
        @ranked, @unranked =
          described_class.rank_next_stratum(stratum, old_ranked, old_unranked)
      end
  ...
  context 'when calling Rcd.move_newly_explained_ercs' do
    context 'with one newly explained erc' do
      ...
      before do
        ...
        @ex_ercs, @unex_ercs =
          described_class.move_newly_explained_ercs(stratum, old_ex_ercs,
                                                    old_unex_ercs)
      end
