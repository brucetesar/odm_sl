...
RSpec.describe Output do
  ...
  context 'given an other output with the same elements and same morphword' do
    before(:example) do
      @output << syl1_stress << syl2_nostress << syl3_unset
      @output.morphword = mword
      @other = Output.new << syl1_stress << syl2_nostress << syl3_unset
  ...
  context 'given an other with differently ordered elements' do
    before(:example) do
      @output << syl1_stress << syl2_nostress << syl3_unset
      @output.morphword = mword
      @other = Output.new << syl3_unset << syl1_stress << syl2_nostress
      @other.morphword = mword
    end
  ...
  context 'given an other with fewer same-ordered elements' do
    before(:example) do
      @output << syl1_stress << syl2_nostress << syl3_unset
      @output.morphword = mword
      @other = Output.new << syl1_stress << syl2_nostress
      @other.morphword = mword
    end
    it 'is not #== to other' do
      expect(@output == @other).to be false
  ...
  context 'given a duplicate' do
    ...
    it 'first syllable is not the same object as the duplicate first syllable' do
