...
RSpec.describe CellRange do
  context 'when created via new()' do
    let(:row_first) { 2 }
    let(:col_first) { 3 }
    let(:row_last) { 6 }
    let(:col_last) { 7 }
    ...
    before do
      @range = described_class.new(row_first, col_first, row_last, col_last)
    end

    it_behaves_like 'Basic CellRange'
  end

  context 'when created via new_from_cells()' do
    let(:row_first) { 1 }
    let(:col_first) { 1 }
    let(:row_last) { 2 }
    let(:col_last) { 2 }
    ...
    before do
      ...
      @range = described_class.new_from_cells(cell1, cell2)
    end
