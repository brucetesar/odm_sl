...
RSpec.describe ErcList do
  context 'when newly created' do
    let(:con_list) { %w[C1 C2] }

    before do
      @erc_list = described_class.new(con_list)
    end
    ...
    it 'raises a RuntimeError when adding an ERC with different constraints' do
      erc_diff = instance_double(Erc, 'erc_diff')
      allow(erc_diff).to receive(:constraint_list).and_return(%w[C3 C4])
    ...
    it 'responds to partition() with an empty true_list' do
      true_list, = @erc_list.partition { true }
      expect(true_list).to be_empty
    end

    it 'responds to partition() with an empty false_list' do
  ...
  context 'with one added erc' do
    ...
    it 'returns an ErcList for #find_all' do
      found = @erc_list.find_all(&:test_cond)
      expect(found).to be_an_instance_of(described_class)
    end
    ...
    it 'partitions into one satisfying ERC' do
      true_list, = @erc_list.partition(&:test_cond)
      expect(true_list.to_a).to contain_exactly(erc1)
    end

    it 'partitions into no non-satisfying ERCs' do
      _, false_list = @erc_list.partition(&:test_cond)
    ...
    context 'with a second erc with the same constraints is added' do
      let(:erc2) { double('erc2') }

      before do
        allow(erc2).to receive(:constraint_list).and_return(%w[C2 C1])
      ...
      it 'partitions into one non-satisfying ERC' do
        _, false_list = @erc_list.partition(&:test_cond)
        expect(false_list.to_a).to contain_exactly(erc2)
      end
    end

    context 'with a second erc with different constraints is added' do
      before do
    ...
    context 'when adding an erc with a different number of constraints' do
      before do
        @erc_diff = instance_double(Erc)
        allow(@erc_diff).to receive(:constraint_list)\
          .and_return(%w[C1 C2 C3])
      end
  ...
  context 'when ERCS are added from a list' do
    let(:con_list) { %w[C1 C2] }
    let(:con_list_diff) { %w[C1 C4] }
    ...
    context 'when the ercs have the same constraints' do
      before do
        allow(@generic_list).to receive(:each).and_yield(@erc_orig)
                                              .and_yield(@erc_same)
        @new_erc_list =
          described_class.new(con_list).add_all(@generic_list)
      end
    ...
    context 'when the ercs have different constraints' do
      before do
        allow(@generic_list).to receive(:each).and_yield(@erc_orig)
                                              .and_yield(@erc_diff)
      end
  ...
  context 'with one consistent ERC added' do
    ...
    before do
      ...
      @erc_list =
        described_class.new(con_list, rcd_runner: @rcd_runner)\
                       .add(@erc_consistent)
  ...
  context 'when created with .new_from_competition' do
    ...
    context 'with a competition of size 2' do
      ...
      before do
        ...
        @erc_list =
          described_class.new_from_competition(winner, competition,
    ...
    context 'with an empty competition' do
      before do
        competition = []
        @erc_list =
          described_class.new_from_competition(winner, competition,
    ...
    context 'with a competition of size 3' do
      let(:loser1) { double('loser1') }
      let(:pair1) { double('wlpair1') }
