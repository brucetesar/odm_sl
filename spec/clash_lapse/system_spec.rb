...
RSpec.describe ClashLapse::System do
  ...
  context 'when the constraints are retrieved' do
    ...
    it 'has 9 constraints' do
      expect(@con_list.size).to eq(9)
    end
    ...
    it 'contains Lapse' do
      expect(@con_list.any? { |c| c.name == 'Lapse' }).to be true
    end
  end
end
