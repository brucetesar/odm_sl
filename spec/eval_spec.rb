...
RSpec.describe Eval do
  ...
  context 'with a competition with only one candidate' do
    let(:cand1) { double('cand1') }

    before do
      @comp = [cand1]
      @eval = described_class.new(comparer)
    end
  ...
  context 'with a more harmonic candidate with others' do
  ...
  context 'with two identical violation candidates that are more harmonic' do
  ...
  context 'with tied candidates that are more harmonic' do
  ...
  context 'with an empty candidate list' do
    before do
      @comp = []
      @eval = described_class.new(comparer)
    end
