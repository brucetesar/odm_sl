...
RSpec.describe 'MorphWord' do
  ...
  context 'when constructed with a root' do
    ...
    context 'and a suffix is added' do
      before(:example) do
        @mw.add(suffix1)
      end
      ...
      it 'each() yields the root followed by the suffix' do
        expect { |probe| @mw.each(&probe) }.to yield_successive_args(root, suffix1)
      end
      ...
      it 'each_with_index() returns a reference to self' do
        @result = @mw.each_with_index { |_obj, _idx| }
        expect(@result).to equal @mw
      end
      it 'returns a string representation of the morphword' do
  ...
  context 'a duplicate' do
    before(:example) do
      @mw = MorphWord.new(root).add(suffix1)
      @dup = @mw.dup
    end
  ...
  context 'when constructed with a non-root' do
    it 'raises a RuntimeError' do
      ...
      expect { MorphWord.new(suffix1) }.to raise_error(RuntimeError, msg)
    end
  end
end
