...
RSpec.describe Input do
  ...
  context 'and a dup' do
    ...
    before(:example) do
      ...
      allow(element_1_1).to receive(:==).and_return(false)
      allow(element_1_1).to receive(:==).with(element_dup_1)\
                                        .and_return(true)
      allow(element_1_2).to receive(:==).and_return(false)
      allow(element_1_2).to receive(:==).with(element_dup_2)\
                                        .and_return(true)
      allow(ui_corr).to receive(:under_corr).with(element_1_1)\
                                            .and_return(uf1)
  ...
  context 'with two segments each with two features' do
    ...
    before(:example) do
      ...
      allow(feature_instance_class).to \
        receive(:new).with(seg1, feat_1_1).and_return(finst_1_1)
      allow(feature_instance_class).to \
        receive(:new).with(seg1, feat_1_2).and_return(finst_1_2)
      allow(feature_instance_class).to \
