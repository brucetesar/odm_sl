...
RSpec.describe OTLearn::RankingBiasSomeLow do
  ...
  context 'when all rankable constraints are the high kind' do
    before do
  ...
  context 'when some rankable constraints are the high kind' do
    before do
      allow(low_kind).to receive(:member?).with(con1).and_return(false)
  ...
  context 'when two low constraints are not active' do
  ...
  context 'when one low kind is active, one is not' do
    let(:erc1) { double('erc1') }
  ...
  context 'when one low kind frees up two high kind, another only frees up one'\
  do
    let(:erc1) { double('erc1') }
    let(:erc2) { double('erc2') }
  ...
  # in the list.
  context 'when one low kind frees up one high kind, another frees up two' do
    let(:erc1) { double('erc1') }
    let(:erc2) { double('erc2') }
  ...
  # TODO: context 'when one low kind frees up a size two high kind cascade'
  # TODO: context 'when no single low kind frees a high kind'
end
