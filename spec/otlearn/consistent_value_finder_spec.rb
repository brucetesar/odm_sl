...
RSpec.describe OTLearn::ConsistentValueFinder do
  ...
  before do
    ...
    @finder = described_class.new(input_assigner: assigner)
    @finder.conflict_evaluator = evaluator
  end

  context 'with a feature with two consistent values' do
  ...
  context 'with a feature with one consistent and one inconsistent value' do
    let(:val1) { double('feature value 1') }
    let(:val2) { double('feature value 2') }
  ...
  context 'with two inconsistent values' do
    let(:val1) { double('feature value 1') }
    let(:val2) { double('feature value 2') }
