...
RSpec.describe OTLearn::SingleFormStep do
  ...
  context 'with a changed grammar that is not all correct' do
    before do
      ...
      @step = described_class.new(test_result, true)
    end

    it 'indicates a step type of SINGLE_FORM' do
      expect(@step.step_type).to eq OTLearn::SINGLE_FORM
    end
  ...
  context 'with an unchanged grammar that is all correct' do
    before do
      ...
      @step = described_class.new(test_result, false)
    end
