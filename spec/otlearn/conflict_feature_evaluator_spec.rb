...
RSpec.describe OTLearn::ConflictFeatureEvaluator do
  ...
  let(:fvp11) { double('feature value pair 11') }
  let(:fvp12) { double('feature value pair 12') }
  let(:fvp21) { double('feature value pair 21') }
  let(:fvp22) { double('feature value pair 22') }
  ...
  before do
    allow(fvp11).to receive(:feature_instance).and_return(finst1)
    allow(fvp12).to receive(:feature_instance).and_return(finst1)
    allow(fvp11).to receive(:alt_value).and_return(value11)
    allow(fvp12).to receive(:alt_value).and_return(value12)
    allow(fvp21).to receive(:feature_instance).and_return(finst2)
    allow(fvp22).to receive(:feature_instance).and_return(finst2)
    allow(fvp21).to receive(:alt_value).and_return(value21)
    allow(fvp22).to receive(:alt_value).and_return(value22)
    allow(assigner).to receive(:assign_input_features)
    @evaluator = described_class.new(input_assigner: assigner,
                                     feature_combiner: combiner)
    @evaluator.consistency_checker = checker
  end

  context 'with a single conflict feature with one consistent value' do
    before do
      @conflict_features = [c_feat1]
      allow(combiner).to receive(:feature_value_combinations)\
        .and_return([[fvp11], [fvp12]])
  ...
  context 'with a single conflict feature with no consistent values' do
    before do
      @conflict_features = [c_feat1]
      allow(combiner).to receive(:feature_value_combinations)\
        .and_return([[fvp11], [fvp12]])
  ...
  context 'with two conflict features with one consistent combination' do
    before do
      @conflict_features = [c_feat1, c_feat2]
      allow(combiner).to receive(:feature_value_combinations)\
        .and_return([[fvp11, fvp21], [fvp11, fvp22], [fvp12, fvp21],
                     [fvp12, fvp22]])
