...
RSpec.describe OTLearn::SingleFormLearning do
  ...
  context 'with one correct winner' do
    ...
    it 'calls set_uf_features once' do
      expect(feature_learner).to\
        have_received(:run).with([win1], grammar).exactly(1).time
    end
    ...
    it 'has step type SINGLE_FORM' do
      expect(@sf_step.step_type).to eq OTLearn::SINGLE_FORM
    end
  end

  context 'with one incorrect winner with a settable feature and other'\
          'unsettable features' do
    let(:winner_list) { [win1] }
    let(:output_list) { [out1] }

    before do
      allow(grammar).to receive(:parse_output).with(out1).and_return(win1)
      allow(feature_learner).to receive(:run).and_return(['feature1'], [])
    ...
    it 'calls set_uf_features twice' do
      expect(feature_learner).to\
        have_received(:run).with([win1], grammar).exactly(2).times
    end
