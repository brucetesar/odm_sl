...
RSpec.describe OTGeneric::CompListReader do
  ...
  context 'with a header array and an array with two competitors' do
    ...
    before do
      ...
      cl_reader = described_class.new(cand_reader: candidate_reader)
    ...
    it 'has two candidates' do
      expect(@comp_list[0].size).to eq 2
    end
  end

  context 'with a data array with 5 rows over 2 inputs' do
    let(:row1) { double('data row 1') }
    let(:row2) { double('data row 2') }
