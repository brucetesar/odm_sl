...
RSpec.describe 'OTGeneric::WinnersReader' do
  ...
  context 'given an array with one winner' do
    before(:example) do
      data = [['in1', 'out11']]
      comp_list = [[cand11, cand12], [cand21, cand22]]
      @win_reader = OTGeneric::WinnersReader.new
  ...
  context 'given an array with two winners' do
    before(:example) do
  ...
  context 'given a winner with input with no corresponding competition' do
    before(:example) do
  ...
  context 'given a winner with no output in its competition' do
    before(:example) do
