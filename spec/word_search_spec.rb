...
RSpec.describe WordSearch do
  context 'when calling #find_unfaithful' do
  ...
  context 'when calling morphemes_to_words' do
    let(:word1) { double('word1') }
  ...
  context 'when calling find_unset_features_of_morpheme' do
    ...
    before do
      ...
      @ws = described_class.new(feat_inst_class: finst_class)
    end
  ...
  context 'when calling conflicting_output_values?' do
    ...
    context 'with conflictng output values' do
      before do
        allow(out_feat1).to receive(:nil?).and_return(false)
        allow(out_feat2).to receive(:nil?).and_return(false)
    ...
    context 'with non-conflicting output values' do
      before do
        allow(out_feat1).to receive(:nil?).and_return(false)
        allow(out_feat2).to receive(:nil?).and_return(false)
        allow(out_feat1).to receive(:value).and_return('value A')
    ...
    context 'with a word lacking an output correspondent' do
      before do
